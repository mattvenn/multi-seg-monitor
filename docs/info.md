...
currently drawing, in a 1 kB line buffer holding four rows at a 256 byte stride.
Each segment is rendered as the AND of an x zone and a y zone within its 15x22
cell, so a digit costs a handful of constant comparisons rather than a bitmap
lookup.
...
- **Stream port** (`uio[7]` high) — a byte on `ui_in` latched on the rising edge of
  ...
  212 bytes per row, 5724 per frame. The write pointer resets on vsync, so the link
...
Pacing falls out of the geometry. A digit row is 22 scanlines and 212 bytes, and
22 x 1056 / 212 = **109.58 pixel clocks per byte**. The remainder never
accumulates: the host clocks its strobe from a fractional divider and both ends
restart on vsync. Four row buffers rather than two let that host run 1-3 rows
ahead without any per-row handshake, and vertical blanking hands it the first
1.23 rows of that lead.
...
The renderer reads 2 of every 15 cycles and the source writes into the gaps, so read
...
reset. You should get a zone plate drawn across the 53x27 grid: concentric rings
...
**Streaming.** Set `uio[7]` high and push 5724 bytes per frame on `ui_in`, one byte
every 109.58 pixel clocks, restarting on each vsync. `tools/video2seg.py` in the project
